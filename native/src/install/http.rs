//! The one place kfxdedrm-fe touches the network.
//!
//! Everything `install` fetches goes through [`Client`], which talks to
//! GitHub and nowhere else. Two things decide its shape:
//!
...
//!   GitHub serves today.
//! - **The crypto provider is RustCrypto's, not `ring`.** That is a decision
//!   about the build rather than about cryptography: `ring` carries a C build
//!   script, Cargo unifies features so it would be *compiled* whether or not a
//!   byte of it were called, and that alone breaks the pure-Rust `rust-lld`
//!   cross path this app's single static binary depends on. See
//!   `native/Cargo.toml`.
//!
//! [`Client::download`] streams to a file and checks `cancel` between chunks,
//! so the panel's Cancel button lands inside a transfer rather than after it.
...
pub const USER_AGENT: &str = concat!(
    ...
    " (+https://github.com/huangziwei/kfxdedrm-fe)"
);

/// An unreachable host should reach the panel quickly rather than looking like
/// a hang.
...
/// Refuse an asset larger than this. The larger of the two is ~5 MB.
...
pub enum Error {
    ...
    Body(String),
    /// The panel's Cancel button, mid-transfer.
    Cancelled,
}
...
impl Client {
    pub fn new() -> Self {
        ...
        // so there is no default to fall back on — which is deliberate: a
        // missing provider should be a wiring decision the compiler asks
        // about, not a runtime surprise.
...
mod tests {
    ...
    fn the_user_agent_names_the_software_and_resolves() {
        assert!(USER_AGENT.starts_with("kfxdedrm-fe/"));
        ...
        assert!(USER_AGENT.contains("github.com/huangziwei/kfxdedrm-fe"));
