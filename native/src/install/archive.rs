...
//! marks what has to be executable — see `install::install_one`.
...
/// carries is a ~5 MB ARM binary; this is the point past which a malformed
/// header should stop us rather than fill the device.
...
fn read_central_directory(file: &mut File) -> Result<Vec<Entry>> {
    ...
    while at + CD_LEN <= cd.len() {
        ...
        // Zip names are bytes; both archives are ASCII and anything else is a
...
mod tests {
    ...
    fn tmpdir(name: &str) -> PathBuf {
        let d = std::env::temp_dir().join(format!("kfxdedrm-fe-archive-{name}"));
        let _ = fs::remove_dir_all(&d);
        fs::create_dir_all(&d).unwrap();
        d
    }

    /// The layout kfxdedrmmobi.zip has: one root folder, a bin/ under it.
    fn sample() -> Vec<u8> {
        let mut z = Zipper::new();
        z.add("kfxdedrm/", b"", false);
        z.add("kfxdedrm/bin/", b"", false);
        z.add("kfxdedrm/bin/kfxdedrmhf_c11", &vec![b'E'; 4096], true);
        z.add("kfxdedrm/config.xml", b"<config/>", false);
        z.finish()
    }
    ...
    fn an_archive_unpacks_out_of_its_own_folder() {
        ...
        let dest = dir.join("kfxdedrm");

        // Two of the four entries are the folders themselves.
        assert_eq!(unpack(&zip, "bin/kfxdedrmhf_c11", &dest).unwrap(), 2);
        assert_eq!(
            fs::read(dest.join("bin/kfxdedrmhf_c11")).unwrap(),
            vec![b'E'; 4096]
        );
    ...
    fn an_archive_without_the_marker_writes_nothing() {
        ...
        let err = unpack(&zip, "bin/bokai", &dest).unwrap_err();
        assert!(matches!(err, Error::NoMarker(_)), "{err}");
        assert!(err.to_string().contains("bin/bokai"));
        assert!(
            !dest.exists(),
    ...
    fn a_damaged_entry_is_caught_rather_than_written() {
        ...
        let err = unpack(&zip, "bin/kfxdedrmhf_c11", &dir.join("out")).unwrap_err();
        assert!(
    ...
    fn the_root_is_whatever_holds_the_marker() {
        // The two real archives, and the two shapes they have.
        let kfx = [
            "kfxdedrm/",
            "kfxdedrm/bin/",
            "kfxdedrm/bin/kfxdedrmhf_c11",
            "kfxdedrm/config.xml",
        ];
        let bokai = [
            "extensions/bokai/",
        ...
        assert_eq!(
            prefix_for(kfx, "bin/kfxdedrmhf_c11"),
            Some("kfxdedrm/".into())
        );
        assert_eq!(
