...
//!
//! GitHub is reached by name, so with no route the resolver runs first and
//! waits out its own timeout before the request fails. Reading
//! `/proc/net/route` answers in a file read instead, which is the difference
//! between a panel that says "turn Wi-Fi on" at once and one that looks hung.
//!
